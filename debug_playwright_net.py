...
async def main():
    ...
    async with async_playwright() as p:
        ...
        async def handle_response(response):
            if "api/projects" in response.url or "supabase.co" in response.url:
                try:
                    text = await response.text()
                    print(f"[NET RESPONSE] {response.request.method} {response.url} -> {response.status} {text[:300]}")
                except Exception as e:
                    print(f"[NET RESPONSE ERROR READING TEXT] {response.request.method} {response.url} -> {response.status}: {e}")
