...
export async function createSheetProject(title?: string): Promise<string | null> {
  try {
    const API = ((typeof window !== 'undefined' && process.env.NODE_ENV === 'production' ? '/backend-proxy' : (process.env.NEXT_PUBLIC_API_URL || 'https://cosmfolio-production.up.railway.app'))).replace(/[^\x20-\x7E]/g, '').trim()
