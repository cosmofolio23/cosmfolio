...
export async function createSheetProject(title?: string): Promise<string | null> {
  try {
    const API = (process.env.NODE_ENV === 'production' ? 'https://cosmfolio-backend.onrender.com' : (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000')).replace(/[^\x20-\x7E]/g, '').trim()
