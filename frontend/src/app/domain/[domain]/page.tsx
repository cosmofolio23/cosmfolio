...
export default function CustomDomainPortfolioPage() {
  ...
  const loadPortfolio = async () => {
    ...
    setError(null)
    try {
      // 1. Resolve domain to project ID
      const domainRes = await fetch(`${API_URL}/api/public/domain/${domain}`)
      if (!domainRes.ok) {
        if (domainRes.status === 404) {
          setError('This domain is not mapped to any public portfolio.')
        } else {
          setError(`Failed to resolve domain: ${domainRes.status}`)
        }
        setIsLoading(false)
        return
      }
      ...
      const projectId = domainData.project_id

      // 2. Fetch pages using the project ID
      const res = await fetch(`${API_URL}/api/portfolios/public/p/${projectId}/pages`, {
        method: 'POST',
      })
      if (!res.ok) {
        setError(`Failed to load portfolio pages: ${res.status}`)
        setIsLoading(false)
        return
      }
  ...
  }, [spreadIndex, totalSpreads, next, prev])
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-800 to-black p-6 text-white">
      <div className="max-w-md text-center">
        ...
        <a href="https://thecosmofolio.com" className="inline-block mt-6 text-white/80 underline text-sm">Create your own portfolio with Cosmofolio →</a>
      </div>
    </div>
  )
